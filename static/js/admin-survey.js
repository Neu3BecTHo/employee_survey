...
class AdminSurveyPage {
    ...
    async init() {
        ...
        this.surveyId = parseInt(urlParts[urlParts.length - 1]);
    ...
    setupEventListeners() {
        // Survey form
        const surveyForm = document.getElementById('edit-survey-form');
        if (surveyForm) {
        ...
        if (editQuestionForm) {
            ...
            if (editQuestionType) {
                editQuestionType.addEventListener('change', (e) => {
                    this.toggleEditOptionsGroup(e.target.value);
                });
            }
        }
    ...
    renderSurvey() {
        document.getElementById('title').value = this.survey.title;
        document.getElementById('description').value = this.survey.description;
        document.getElementById('status').value = this.survey.status;
        
        // Disable add question button for non-draft surveys
        const addQuestionBtn = document.getElementById('add-question-btn');
        if (addQuestionBtn) {
            if (this.survey.status !== 'draft') {
                addQuestionBtn.disabled = true;
                addQuestionBtn.textContent = '📝 Добавлять вопросы можно только в черновики';
                addQuestionBtn.title = 'Чтобы добавить вопросы, измените статус опроса на "Черновик"';
            } else {
            ...
            <div class="question-card" data-question-id="${question.id}">
                <div class="question-header">
                    ...
                    <div class="question-actions">
                        ${this.survey.status === 'draft' ? `
                            <button class="btn btn-secondary btn-sm" onclick="adminSurveyPage.editQuestion(${question.id})">
                                ✏️ Редактировать
                            </button>
                            <button class="btn btn-danger btn-sm" onclick="adminSurveyPage.deleteQuestion(${question.id})">
                                🗑️ Удалить
                            </button>
                        ` : ''}
                    </div>
                ...
                <div class="question-content">
                    ...
                    <div class="question-meta">
                        ...
                        ${question.is_required ? '<span class="required-badge">Обязательный</span>' : ''}
                    </div>
                    ${question.options ? `
    ...
    async updateSurvey() {
        try {
            ...
            this.showMessage('Опрос успешно обновлен!', 'success');
    ...
    async addQuestion() {
        try {
            const formData = new FormData(document.getElementById('add-question-form'));
            const data = {
            ...
            if (data.type === 'single_choice') {
                ...
                if (data.options.length < 2) {
                    this.showMessage('Для вопроса с выбором варианта нужно минимум 2 варианта ответа', 'error');
                    return;
                }
            }
    ...
    editQuestion(questionId) {
        ...
        if (question.type === 'single_choice' && question.options) {
            ...
            question.options.forEach((option, index) => {
                ...
                optionDiv.className = 'option-input';
                optionDiv.innerHTML = `
                    <input type="text" name="edit_option_${index}" value="${option}" placeholder="Вариант ответа ${index + 1}" required>
                    <button type="button" class="btn btn-danger btn-sm" onclick="this.parentElement.remove()">🗑️</button>
                `;
                container.appendChild(optionDiv);
            });
        ...
        this.toggleEditOptionsGroup(question.type);
    ...
    addEditOption() {
        ...
        optionDiv.className = 'option-input';
        optionDiv.innerHTML = `
            <input type="text" name="edit_option_${optionCount}" placeholder="Вариант ответа ${optionCount + 1}" required>
            <button type="button" class="btn btn-danger btn-sm" onclick="this.parentElement.remove()">🗑️</button>
        `;
        container.appendChild(optionDiv);
    }
    ...
    async updateQuestion() {
        try {
            const formData = new FormData(document.getElementById('edit-question-form'));
            const questionId = parseInt(formData.get('question-id'));
            const data = {
                text: formData.get('text'),
                type: formData.get('type'),
                is_required: formData.has('is_required')
            ...
            if (data.type === 'single_choice') {
                data.options = [];
                const options = document.querySelectorAll('#edit-options-container input[type="text"]');
                options.forEach(input => {
                    if (input.value.trim()) {
                        data.options.push(input.value.trim());
                    }
            ...
            this.showMessage('Вопрос успешно обновлен!', 'success');
            this.hideEditQuestionModal();
            await this.loadQuestions();
        ...
        this.showConfirmModal(
            ...
            async () => {
                try {
                    ...
                    this.showMessage('Вопрос успешно удален!', 'success');
                    await this.loadQuestions();
...
document.addEventListener('DOMContentLoaded', () => {
    window.adminSurveyPage = new AdminSurveyPage();
});
